...
const Header = () => (
  <header className="relative text-center mb-12 md:mb-20 px-4 z-10">
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-secondary rounded-full filter blur-3xl opacity-20 -z-10" aria-hidden="true"></div>
    ...
    <h1 className="text-4xl sm:text-5xl md:text-6xl font-quicksand font-bold leading-tight tracking-tighter animate-fade-in-down">
      <span className="text-transparent bg-clip-text bg-gradient-to-r from-accent to-orange-500">{title}</span>
    </h1>

    <p className="text-secondary mt-6 text-base sm:text-lg max-w-2xl mx-auto animate-fade-in-down" style={{animationDelay: '0.4s'}}>
        {description}
    </p>
  </header>
);
...
const PricingCard = ({ plan, currency = '$', price, description, features, isFeatured = false }: PricingCardProps) => (
  <article
  ...
  >
    ...
    <div className="mb-6 flex items-baseline gap-1.5">
      {price === 0 ? (
        ...
        <>
          <span className={`text-4xl sm:text-5xl font-bold ${isFeatured ? 'text-white' : 'text-gray-900 dark:text-white'}`}>{currency}{price}</span>
...
export function Pricing4() {
  ...
  return (
    <>
      ...
      `}</style>
      <div className="relative font-sans flex flex-col items-center justify-center overflow-x-hidden bg-primary min-h-screen">
